...
<?php
  ...
  $sql= " SELECT * FROM info_acc WHERE id_cat_type=4 ";
...
<section class="contenedor seccion seccion-vendidos">
  <h2>Para llevar todo eso que te falta</h2>

<?php 
  if(!$_GET){
    header('Location:mochilas&bolsos.php?pagina=1');
  }
  $iniciar = ($_GET['pagina']-1) * $pag;

  $sql_articulos = " SELECT * FROM info_acc WHERE id_cat_type=4 LIMIT :iniciar, :narticulos ";
...
<nav class="">
  <ul class="list-pag">
    ...
    <i class="fas fa-arrow-circle-left"></i>
      <a class="but" href="mochilas&bolsos.php?pagina=<?php echo $_GET['pagina']-1; ?>">
        Anterior
      </a>
    </li>
    ...
    <li class="page-item <?php echo $_GET['pagina']== $i + 1 ? 'activos' : ''; ?>">
      <a class="but " href="mochilas&bolsos.php?pagina=<?php echo $i + 1; ?>">
        <?php echo $i + 1; ?>
      </a>
    </li>
