...
<?php
  ...
  $sql= " SELECT * FROM info_cloth WHERE id_cat_type=3 ";
...
<ul class="section-resumen" >
  ...
  <li><a href="contavientoman.php?pagina=1" >Cortaviento</a></li>
</ul>
</div>
...
<?php 
  if(!$_GET){
    header('Location:cortavientoman.php?pagina=1');
  }
  $iniciar = ($_GET['pagina']-1) * $pag;

  $sql_articulos = " SELECT * FROM info_cloth WHERE id_cat_type=3 LIMIT :iniciar, :narticulos ";
...
<nav class="">
  <ul class="list-pag">
    ...
    <i class="fas fa-arrow-circle-left"></i>
      <a class="but" href="cortavientoman.php?pagina=<?php echo $_GET['pagina']-1; ?>">
        Anterior
      </a>
    </li>
    ...
    <li class="page-item <?php echo $_GET['pagina']== $i + 1 ? 'activos' : ''; ?>">
      <a class="but " href="cortavientoman.php?pagina=<?php echo $i + 1; ?>">
        <?php echo $i + 1; ?>
      </a>
    </li>
